...
do
    hdfs dfs -cat -ignoreCrc -ignoreCrc $IN/$input | grep 'the land of' | pure_func ${input} | sort -nr | sed 5q > ${OUT}/${input}.out0
    hdfs dfs -cat -ignoreCrc -ignoreCrc $IN/$input | grep 'And he said' | pure_func ${input} | sort -nr | sed 5q > ${OUT}/${input}.out1
done
