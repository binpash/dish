...
do
    hdfs dfs -cat -ignoreCrc $IN/$input | grep 'the land of' | pure_func ${input} | sort -nr | sed 5q > ${OUT}/${input}.0.out
    hdfs dfs -cat -ignoreCrc $IN/$input | grep 'And he said' | pure_func ${input} | sort -nr | sed 5q > ${OUT}/${input}.1.out
done
